...
def _usage():
	...
	print("                                   (see config_reweight_example.yaml for template)")
	print("")
	print("  Legacy Command-Line Arguments (for backward compatibility):")
	print("       %s T dim_UMB periodicity_UMB harmonicBiasesFile_UMB trajDir_UMB histBinEdgesFile_UMB histDir_UMB fMAPFile_UMB fMHFile_UMB trajDir_PROJ histBinEdgesFile_PROJ [kB]" % sys.argv[0])
	print("")
...
def load_config_from_yaml(config_file):
	...
	params = {}
	params['T'] = config['temperature']
	params['kB'] = config.get('boltzmann_constant', 0.0083144621)  # Default: kJ/mol·K
	params['dim_UMB'] = config['umbrella']['dim']
	params['periodicity_UMB'] = config['umbrella']['periodicity']
	params['harmonicBiasesFile_UMB'] = config['umbrella']['files']['harmonic_biases']
	params['trajDir_UMB'] = config['umbrella']['files']['trajectory_dir']
	params['histBinEdgesFile_UMB'] = config['umbrella']['files']['hist_bin_edges']
	params['histDir_UMB'] = config['umbrella']['files']['hist_dir']
	params['fMAPFile_UMB'] = config['umbrella']['files']['f_map']
	params['fMHFile_UMB'] = config['umbrella']['files']['f_mh']
	params['trajDir_PROJ'] = config['projection']['files']['trajectory_dir']
	params['histBinEdgesFile_PROJ'] = config['projection']['files']['hist_bin_edges']
...
def binner(valVec,binE):
	...
	for d in range(0,dim):
		if ( ( valVec[d] < binE[d][0] ) or ( valVec[d] >= binE[d][-1] ) ):
		...
		for i in range(0,len(binE[d])):
			if ( ( valVec[d] >= binE[d][i] ) and ( valVec[d] < binE[d][i+1] ) ):
				sub[d] = i
				break
...
def main():
	"""Main entry point for BayesReweight analysis."""


	# parameters
	# (kB will be set from config or CLI args)
	...
	if len(sys.argv) == 3 and sys.argv[1] == '--config':
		...
		T = params['T']
		kB = params['kB']
		dim_UMB = params['dim_UMB']
		periodicity_UMB = params['periodicity_UMB']
		harmonicBiasesFile_UMB = params['harmonicBiasesFile_UMB']
		trajDir_UMB = params['trajDir_UMB']
		histBinEdgesFile_UMB = params['histBinEdgesFile_UMB']
		histDir_UMB = params['histDir_UMB']
		fMAPFile_UMB = params['fMAPFile_UMB']
		fMHFile_UMB = params['fMHFile_UMB']
		trajDir_PROJ = params['trajDir_PROJ']
		histBinEdgesFile_PROJ = params['histBinEdgesFile_PROJ']
	...
	elif len(sys.argv) == 12 or len(sys.argv) == 13:
		# Legacy CLI arguments mode (11 or 12 args)
		T = float(sys.argv[1])
		dim_UMB = int(sys.argv[2])
		periodicity_UMB = str(sys.argv[3])
		harmonicBiasesFile_UMB = str(sys.argv[4])
		trajDir_UMB = str(sys.argv[5])
		histBinEdgesFile_UMB = str(sys.argv[6])
		histDir_UMB = str(sys.argv[7])
		fMAPFile_UMB = str(sys.argv[8])
		fMHFile_UMB = str(sys.argv[9])
		trajDir_PROJ = str(sys.argv[10])
		histBinEdgesFile_PROJ = str(sys.argv[11])
		# Optional kB parameter (12th argument)
		if len(sys.argv) == 13:
			kB = float(sys.argv[12])
		else:
			kB = 0.0083144621  # Default: kJ/mol·K

		# Parse periodicity from string format
		periodicity_UMB = periodicity_UMB[1:-1].split(',')
		periodicity_UMB = [int(x) for x in periodicity_UMB]
	...
	# Handle periodicity type (for YAML mode where it's already a list)
	if isinstance(periodicity_UMB, str):
		periodicity_UMB = periodicity_UMB[1:-1].split(',')
		periodicity_UMB = [int(x) for x in periodicity_UMB]
	...
	with open(harmonicBiasesFile_UMB,'r') as fin:
		for line in fin:
			harmonicBiasesFile_UMB_DATA.append(line.strip().split())
	...
	n_il = [[int(y) for y in x] for x in n_il]
	n_il = np.array(n_il)
	...
	fin = open(fMAPFile_UMB,'r')
	line = fin.readline()
	f_i_MAP = line.strip().split()
	f_i_MAP = [float(x) for x in f_i_MAP]
	f_i_MAP = np.array(f_i_MAP)
	fin.close()

	if f_i_MAP.shape[0] != S:
	...
	f_i_MH = []
	with open(fMHFile_UMB,'r') as fin:
		for line in fin:
			f_i_MH.append(line.strip().split())
	f_i_MH = [[float(y) for y in x] for x in f_i_MH]
	f_i_MH = np.array(f_i_MH)
	...
	for i in range(0,S):
		...
		traj_i_DATA = []
		with open(traj_i_filename,'r') as fin:
			for line in fin:
				traj_i_DATA.append(line.strip().split())
		traj_i_DATA = [[float(y) for y in x] for x in traj_i_DATA]
		traj_i_DATA = np.array(traj_i_DATA)
	...
	for i in range(0,S):
		...
		traj_PROJ_i_DATA = []
		with open(traj_PROJ_i_filename,'r') as fin:
			for line in fin:
				traj_PROJ_i_DATA.append(line.strip().split())
		traj_PROJ_i_DATA = [[float(y) for y in x] for x in traj_PROJ_i_DATA]
		traj_PROJ_i_DATA = np.array(traj_PROJ_i_DATA)
		if i == 0:
	...
	for i in range(0,S):
		for k in range(0,N_i[i]):
			...
			# - bound checking
			for d in range (0,dim_PROJ):
				if ( ( traj_PROJ_ik[d] < binE_PROJ[d][0] ) or ( traj_PROJ_ik[d] >= binE_PROJ[d][-1] ) ):
			...
			# - bound checking
			for d in range (0,dim_UMB):
				if ( ( traj_ik[d] < binE[d][0] ) or ( traj_ik[d] >= binE[d][-1] ) ):
	...
	betaF_l_PROJ_MAP = betaF_l_PROJ_MAP - np.mean(betaF_l_PROJ_MAP[np.isfinite(betaF_l_PROJ_MAP)])
	...
	with open("hist_binCenters_PROJ.txt","w") as fout:
		for d in range(0,dim_PROJ):
			for ii in range(0,M_k_PROJ[d]):
				fout.write("%15.5e" % (binC_PROJ[d][ii]))
			fout.write("\n")

	with open("hist_binWidths_PROJ.txt","w") as fout:
		for d in range(0,dim_PROJ):
			for ii in range(0,M_k_PROJ[d]):
	...
	with open("p_PROJ_MAP.txt","w") as fout:
		for l in range(0,M_PROJ):
			fout.write("%15.5e" % (p_l_PROJ_MAP[l]))
		fout.write("\n")

	with open("pdf_PROJ_MAP.txt","w") as fout:
		for l in range(0,M_PROJ):
			fout.write("%15.5e" % (pdf_l_PROJ_MAP[l]))
		fout.write("\n")

	with open("betaF_PROJ_MAP.txt","w") as fout:
		for l in range(0,M_PROJ):
	...
	betaF_l_PROJ_MH_colMean = np.ones(nSamples_MH)*float('nan')
	for jj in range(0,nSamples_MH):
		betaF_l_PROJ_MH_col = betaF_l_PROJ_MH[:,jj]
		betaF_l_PROJ_MH_colMean[jj] = np.mean(betaF_l_PROJ_MH_col[np.isfinite(betaF_l_PROJ_MH_col)])
	betaF_l_PROJ_MH = betaF_l_PROJ_MH - np.matlib.repmat( betaF_l_PROJ_MH_colMean, betaF_l_PROJ_MH.shape[0], 1 )
	...
	with open("p_PROJ_MH.txt","w") as fout:
		for jj in range(0,nSamples_MH):
			for l in range(0,M_PROJ):
				fout.write("%15.5e" % (p_l_PROJ_MH[l,jj]))
			fout.write("\n")

	with open("pdf_PROJ_MH.txt","w") as fout:
		for jj in range(0,nSamples_MH):
			for l in range(0,M_PROJ):
				fout.write("%15.5e" % (pdf_l_PROJ_MH[l,jj]))
			fout.write("\n")

	with open("betaF_PROJ_MH.txt","w") as fout:
		for jj in range(0,nSamples_MH):
			for l in range(0,M_PROJ):
